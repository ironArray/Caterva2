...
* Web frontend: Improved navigation and display of dataset information.
* Web frontend: New Data tab to show the dataset's data. It supports NDArrays with structured dtypes as tables.
* Web frontend: Support for displaying MarkDown files.
* Web frontend: Support for authentication. See https://demo-auth.caterva2.net for an example.
* Web frontend: New @personal area (pseudo-root) when logged in. It allows creating new datasets and uploading files (drag-and-drop supported).
* Web frontend: New Prompt box to allow creating lazy datasets in the @personal area. Existing datasets can be referenced by their tag.
* Web frontend: New Download button to download a dataset as a file.
* Web frontend: New Delete button to delete a dataset from @personal area.
* Web frontend: Support to detect tomographies automatically using heuristics -- 3D integer datasets in greyscale and RGB(A) (using a 4th dim).
...
* Web frontend: Allow listing the datasets of multiple roots.
* Web frontend: Add support for display plugins that depend on the `contenttype` attribute of the dataset.
* Web frontend: Add tomography display plugin that allows choosing and rendering any 2D image in a 3D stack (along the first dimension).
* Web frontend: Use tabs to switch between dataset info and other features (like displays).
* Web frontend: Show dataset vlmeta (user attributes) in info tab.
